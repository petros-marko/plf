...
Definition refines (c1 c2 : com) : Prop := forall (st st' : state),
    (st =[ c1 ]=> st') -> (st =[ c2 ]=> st').
...
Proof.
  ...
  - inversion H; subst.
    + apply E_IfFalse.
      * simpl in H5.
        replace (beval st b) with (negb (negb (beval st b))).
...
Hint Constructors ceval : ceval_db.
...
Theorem assign_aequiv : forall (X : string) (a : aexp),
  ...
  cequiv <{ skip }> <{ X := a }>.
Proof.
  intros. unfold aequiv in H. split; intros.
  - simpl in H.
    assert (H': st = (X !-> aeval st a; st)).
    {
      rewrite <- H. rewrite t_update_same. reflexivity.
    }
    inversion H0; subst.
    replace (st' =[ X := a ]=> st') with (st' =[ X := a ]=> (X !-> aeval st' a; st'));
    [eapply E_Asgn | rewrite <- H'];
    reflexivity.
  - simpl in H.
    inversion H0; subst.
    rewrite <- H, t_update_same.
    constructor.
Qed.
(** [] *)
...
Proof.
  ...
  - (* while *)
    destruct (fold_constants_bexp b) eqn:B';
    try (apply while_true; rewrite <- B'; apply fold_constants_bexp_sound);
    try (apply while_false; rewrite <- B'; apply fold_constants_bexp_sound);
    try (
      apply CWhile_congruence ; [ 
        rewrite <- B'; apply fold_constants_bexp_sound |
        assumption
      ]
    ).
  Qed.
...
Proof.
  ...
  try reflexivity.
  - simpl.
    destruct (optimize_0plus_aexp a1) eqn:A1;
      try destruct n;
...
Proof.
  ...
  induction b;
  try reflexivity;
  try (simpl; rewrite <- optimize_0plus_aexp_sound, <- optimize_0plus_aexp_sound; reflexivity).
...
Proof.
  ...
  induction c; simpl;
  try reflexivity;
  [ apply CAsgn_congruence | apply CSeq_congruence |
    apply CIf_congruence | apply CWhile_congruence ];
...
Proof.
  intros. induction H;
  try (simpl; rewrite IHvar_not_used_in_aexp1, IHvar_not_used_in_aexp2);
  try reflexivity.
...
Theorem fvar_not_in_rhs_subst_equiv_property : forall x1 x2 a1 a2,
...
Proof.
  ...
  eapply E_Seq; try eassumption.
  {
    ...
    induction a2.
    - reflexivity.
    - destruct (String.eqb x1 x) eqn:Ex1x.
      + simpl. rewrite Ex1x.
        inversion H3; subst. rewrite String.eqb_eq in Ex1x. rewrite <- Ex1x.
        assert (H' : aeval (x1 !-> aeval st a1; st) a1 = aeval st a1).
          { rewrite aeval_weakening; auto. }
        rewrite H'. unfold t_update. rewrite String.eqb_refl. reflexivity.
      + simpl. rewrite Ex1x. reflexivity.
    - simpl.
      rewrite IHa2_1,IHa2_2; auto using E_Asgn.
    - simpl.
      rewrite IHa2_1,IHa2_2; auto using E_Asgn.
    - simpl.
      rewrite IHa2_1,IHa2_2; auto using E_Asgn.
...
Proof.
  ...
  assert (A: (_ !-> 0) =[ pcopy ]=> (Y !-> 2; X !-> 1; (_ !-> 0))).
  {
    subst. apply H. unfold ptwice.
  ...
  subst.
  inversion H13.
  assert (E1n : (Y !-> aeval (X !-> n; _ !-> 0) X;
X !-> n; _ !-> 0) X = (Y !-> 2; X !-> 1; _ !-> 0) X).
    { 
      rewrite H13. reflexivity.
    }
   assert (E2n : (Y !-> aeval (X !-> n; _ !-> 0) X;
X !-> n; _ !-> 0) Y = (Y !-> 2; X !-> 1; _ !-> 0) Y).
    { 
      rewrite H13. reflexivity.
    }
  assert (YnX : Y <> X).
  ...
  clear H5. clear H2. clear H13. clear H1.
  assert (Contra : 1 = 2).
  {
    simpl in E2n.
    replace ((X !-> n; _ !-> 0) X) with n in E2n by (unfold t_update; rewrite String.eqb_refl; reflexivity).
    unfold t_update in E2n. rewrite String.eqb_refl in E2n.
    simpl in E1n.
    replace ((X !-> n; _ !-> 0) X) with n in E1n by (unfold t_update; rewrite String.eqb_refl; reflexivity).
    unfold t_update in E1n. rewrite String.eqb_refl in E1n. rewrite <- String.eqb_neq in YnX.
    rewrite YnX in E1n. rewrite <- E2n, E1n. reflexivity.
  }
  inversion Contra.
  Qed.
...
(* If for all states where b evaluates to true it is the case that
   evaluating c gets us to a state where b also evaluates to true,
   then <{ while b do c end }> cannot take st to any state st'
*)
Lemma while_noterm_step : forall b c,
  (forall st st', beval st b = true -> st =[ c ]=> st' -> beval st' b = true) ->
  (forall st st', beval st b = true -> ~ st =[ <{ while b do c end }> ]=> st').
Proof.
  intros. unfold not. intros.
  remember (<{ while b do c end }>) as e.
  induction H1;
  try inversion Heqe; subst.
...
Lemma p1_may_diverge : forall st st', st X <> 0 ->
  ~ st =[ p1 ]=> st'.
Proof.
  unfold p1. intros st st'.
  intros N. rewrite <- eqb_neq in N.
  apply while_noterm_step.
  - intros. simpl.
    ...
    rewrite <- eqb_neq in N'. rewrite N'. reflexivity.
...
Proof.
  ...
  destruct (beval st <{ ~ X = 0}>) eqn:B.
  - assert (XS: st X <> 0).
      { apply eqb_neq. apply negb_true_iff. simpl in B. assumption. }
    split; intros.
    + eapply p1_may_diverge in XS. apply XS in H. destruct H.
    + eapply p2_may_diverge in XS. apply XS in H. destruct H.
  - unfold p1, p2. split; intros;
    (
      inversion H; subst; [
        eapply E_WhileFalse; assumption |
        rewrite B in H2; discriminate H2
      ]
    ).
Qed.
...
Proof.
  ...
  specialize H with 
    ...
    (st' := (Z !-> 2 ; X !-> 0 ; Z !-> 1 ; X !-> 1)).
  destruct H. clear H0.
  assert (H' : (X !-> 1) =[ p4 ]=> (Z !-> 2; X !-> 0; Z !-> 1 ; X !-> 1)).
    { 
      apply H. unfold p3.
      eapply E_Seq.
      - apply E_Asgn. reflexivity.
  ...
  assert (Contra : 1 = 2).
    { 
      replace 2 with ((Z !-> 2; X !-> 0; Z !-> 1; X !-> 1) Z) by reflexivity.
      replace (Z !-> 2; X !-> 0; Z !-> 1; X !-> 1) with (Z !-> 1; X !-> 0; X !-> 1) by H4.
...
Proof.
  unfold cequiv. intros. split; intros.
  {
    remember p5 as p.
    induction H;
    try inversion Heqp; subst.
    - simpl in H. rewrite negb_false_iff in H. rewrite eqb_eq in H.
      unfold p6.
      replace (st =[ X := 1]=> st) with (st =[ X := 1]=> (X !-> 1; st))
        by (rewrite <- H; rewrite t_update_same; reflexivity).
      apply E_Asgn. reflexivity.
  ...
  }
  {
    unfold p5.
    inversion H; subst.
    destruct (eqb (st X) 1) eqn:X.
    - rewrite eqb_eq in X.
      simpl. rewrite <- X. rewrite t_update_same.
      apply E_WhileFalse. simpl. rewrite X. reflexivity.
    - simpl.
      eapply E_WhileTrue.
...
Proof.
  ...
  intros. split; intros.
  {
    ...
    rewrite aeval_weakening; [idtac | assumption].
    eapply E_Seq.
    - eapply E_Asgn. reflexivity.
    - rewrite (t_update_permute _ _ _ _ _ _ H).
      replace (l1 !-> aeval st a1; l2 !-> aeval st a2; st) 
        with (l1 !-> aeval (l2 !-> aeval st a2; st) a1; l2 !-> aeval st a2; st)
        by (rewrite aeval_weakening; auto).
      apply E_Asgn. reflexivity.
  }
  {
    inversion H2; inversion H5; inversion H8; subst.
    ...
    - apply not_eq_sym in H.
      ...
      replace (l2 !-> aeval st a2; l1 !-> aeval st a1; st) 
        with (l2 !-> aeval (l1 !-> aeval st a1; st) a2; l1 !-> aeval st a1; st)
        by (rewrite aeval_weakening; auto).
      apply E_Asgn. reflexivity.
  }
  Qed.
...
Proof.
  ...
  split; intros.
  {
    ...
    assert (H': st =[ c3 ]=> st').
      { unfold c3. rewrite Heqst',Heqst. apply E_Asgn. reflexivity. }
    apply H in H'.
    inversion H'; subst n; subst a; subst x; subst st0.
    simpl in H3.
    assert (Contra : 1 = 2).
      {
        replace 2 with (st' X) by (rewrite <- H3; reflexivity).
        replace 1 with (st' X) by (rewrite Heqst'; reflexivity).
        reflexivity.
      }
    discriminate Contra.
  }
  {
    remember (_ !-> 0) as st.
    remember (X !-> 2; st) as st'.
    assert (H' : st =[ c4 ]=> st').
      { unfold c4. rewrite Heqst',Heqst. apply E_Asgn. reflexivity. }
    apply H in H'.
    inversion H'; subst n; subst a; subst x; subst st0.
    simpl in H3.
    assert (Contra : 1 = 2).
      {
        replace 2 with (st' X) by (rewrite Heqst'; reflexivity).
        replace 1 with (st' X) by (rewrite <- H3; reflexivity).
        reflexivity.
      }
    discriminate Contra.
  }
...
Proof.
  intros. unfold capprox, cmin.
  intros. apply while_true_nonterm in H.
  - contradiction.
  - apply refl_bequiv.
  Qed.
...
Definition zprop_interesting (c : com): Prop := cequiv c <{ c ; X := 1 }>.

Theorem zprop_interesting_preserving : forall c c',
  zprop_interesting c -> capprox c c' -> zprop_interesting c'.
Proof.
  Admitted.
