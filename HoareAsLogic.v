...
Theorem provable_true_post : forall c P,
    derivable P c True.
Proof.
  intros. apply H_Consequence_pre with (P' := fun _ => True).
  - induction c.
    + constructor.
    + eapply H_Consequence with (P' := (fun _ => True)[x |-> a]);
      constructor.
...
Proof.
  intros.
  apply H_Consequence_post with (Q' := (fun _ => False)).
  {
    induction c.
    - constructor.
    - apply H_Consequence_pre with (P' := (fun _ => False)[ x |-> a ]).
      + constructor.
    ...
    - eapply H_Consequence_post.
      + apply H_While.
        eapply H_Consequence_pre.
        * eassumption.
        * simpl. intros. repeat destruct H.
      + simpl. intros. repeat destruct H.
  }
  simpl. intros. destruct H.
  Qed.
...
Proof.
  ...
  - apply H_If.
    + eapply IHc1 with (P := (P /\ b)%assertion).
      intros. destruct H0.
      eapply HT; eauto.
    + eapply IHc2 with (P := (P /\ ~ b)%assertion).
      intros. destruct H0.
      eapply HT.
      * apply E_IfFalse.
        ** simpl in H1.
  ...
  - eapply H_Consequence.
    + apply H_While with (P := (wp <{while b do c end}> Q)%assertion).
      apply IHc. unfold wp. intros.
      destruct H0. eauto.
    + unfold wp. intros.
      eapply HT; eauto.
    + unfold wp. intros.
