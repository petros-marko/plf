...
Proof.
  verify;
  try (destruct (st X) as [| [| n]]; lia);
  rewrite <- H;
    [ apply parity_ge_2 | symmetry; apply parity_lt_2 ];
  assumption.
  Qed. 
...
Proof.
  verify.
  - rewrite mul_comm, add_1_r.
    reflexivity.
  - rewrite eq_dne in H1. rewrite H1.
    reflexivity.
  Qed.
(** [] *)
...
Definition minimum_dec (a b : nat) : decorated :=
  <{
    ...
    {{ a = a }}
      ...
      while X <> 0 && Y <> 0 do
             {{ X + Z = a /\ Y + Z = b /\ X <> 0 /\ Y <> 0 }} ->>
             {{ (X - 1) + 1 + Z = a /\ Y + Z = b /\ X <> 0 /\ Y <> 0 }}
        X := X - 1
             {{ X + 1 + Z = a /\ (Y - 1) + 1 + Z = b /\ Y <> 0 }};
        Y := Y - 1
             {{ X + 1 + Z = a /\ Y + 1 + Z = b }};
        Z := Z + 1
             {{ X + Z = a /\ Y + Z = b }}
...
Proof.
  ...
  try (
    ...
    rewrite negb_true_iff, eqb_neq in *;
    assumption
  ).
  unfold not. intros. destruct H.
  apply andb_false_iff in H0.
  destruct H0 as [Hx | Hy];
  rewrite negb_false_iff, eqb_eq in *;
  congruence.
  Qed.
(** [] *)
...
Definition dpow2_dec (n : nat) :=
  <{
    {{ True }}
      ...
      Z := 1
               {{ X = 0 /\ Y = 1 /\ Z = 1 }} ->>
               {{ Y + 1 = ap pow2 (X + 1) /\ Z = ap pow2 X}};
      while X <> n do
               {{ Y + 1 = ap pow2 (X + 1) /\ Z = ap pow2 X /\ X <> n }} ->>
               {{ Y + 1 = ap pow2 (X + 1) /\ 2 * Z = ap pow2 (X + 1) }}
        Z := 2 * Z 
               {{ Y + 1 = ap pow2 (X + 1) /\ Z = ap pow2 (X + 1) }} ->>
               {{ Y + Z + 1 = ap pow2 ((X + 1) + 1) /\ Z = ap pow2 (X + 1) }};
        Y := Y + Z
               {{ Y + 1 = ap pow2 ((X + 1) + 1) /\ Z = ap pow2 (X + 1) }};
        X := X + 1
               {{ Y + 1 = ap pow2 (X + 1) /\ Z = ap pow2 X }}
      end
    {{ Y + 1 = ap pow2 (X + 1) /\ Z = ap pow2 X /\ ~ (X <> n) }} ->>
...
Proof.
  ...
  - replace (pow2 (st X + 1 + 1)) 
      with (pow2 (st X + 1) + pow2 (st X + 1))
      by (rewrite pow2_plus_1, pow2_plus_1, pow2_plus_1; reflexivity).
    rewrite <- H. lia.
  - rewrite eq_dne in H1. rewrite H1 in H.
    rewrite <- H. lia.
  Qed.
(** [] *)
...
Proof.
  ...
  - apply IHn in H1 as H2.
    rewrite add_1_l in H2. rewrite <- H2.
    simpl. destruct n eqn:N.
    + inversion H1.
    + reflexivity.
  Qed.
(** [] *)
...
Definition dfib (n : nat) : decorated :=
  <{
    ...
    while X <> 1 + n do
                  {{X > 0 /\ Y = ap fib (ap pred X) /\ Z = ap fib X }}
      T := Z
                  {{ X > 0 /\ Y = ap fib (ap pred X) /\ T = ap fib X /\ Z = ap fib X }} ->>
                  {{ X > 0 /\ Y = ap fib (ap pred X) /\ T = ap fib X /\ Z + Y = ap fib X + ap fib (ap pred X) }};
      Z := Z + Y
                  {{ X > 0 /\ Y = ap fib (ap pred X) /\ T = ap fib X /\ Z = ap fib (1 + X) }};
      Y := T
                  {{ X > 0 /\ Y = ap fib (ap pred (1 + X)) /\ Z = ap fib (1 + X) }};
      X := 1 + X
                  {{ X > 0 /\ Y = ap fib (ap pred X) /\ Z = ap fib X }}
    end
    {{ Y = ap fib (ap pred X) /\ ~ (X <> 1 + n) }} ->>
    {{ Y = fib n }}
   }>.
...
Proof.
  verify.
  - rewrite H2. rewrite fib_eqn; auto.
  - rewrite eq_dne in H0. rewrite H0.
    reflexivity.
  Qed.
...
Proof.
  ...
  }
  {
    ...
    assert (Asgn : st =[ X := Y + 1]=> st').
      { subst st'. apply E_Asgn. reflexivity. }
    apply H in Asgn; subst st'.
    - unfold t_update in Asgn.
      rewrite String.eqb_refl in Asgn.
      simpl in Asgn.
      apply le_add_le_sub_r in Asgn.
      apply Asgn.
    - assumption.
  }
  Qed.
...
Lemma hoare_havoc_weakest : forall (P Q : Assertion) (X : string),
  ...
  P ->> havoc_pre X Q.
Proof.
  unfold "->>", havoc_pre, assertion_sub, valid_hoare_triple.
  intros.
  specialize H with
    (st := st)
    (st' := (X !-> aeval st n ; st)).
