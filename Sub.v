...
(** **** Exercise: 2 stars, standard, especially useful (arrow_sub_wrong)
    ...
    f = \ x: Student, x.gpa
    g = \ f: Person -> Nat, f { name = Bill, age = 24 }
    g f gets stuck because
    { name = Bill, age = 24}.gpa cannot take a step
*)
...
(** **** Exercise: 1 star, standard (subtype_concepts_tf)
    ...
    - There is an infinite descending chain of distinct types in the
      ...
      true
      { a : Nat } :> { a : Nat, b : Nat } :> { a : Nat, b : Nat, c : Nat } ...
...
(** **** Exercise: 2 stars, standard (proper_subtypes)
    ...
    false; consdider the type T = Bool * Bool.
    S <: T can only happen if S is a product type T1 * T2 where
    T1 <: Bool and T2 <: Bool. But Bool has no proper subtypes, so
    T1 = T2 = Bool and hence S = Bool * Bool = T
*)
...
Proof with eauto.
  ...
  try solve_by_invert...
  - remember (IHHs2 V1 V2 eq_refl) as UeF. destruct HeqUeF.
    destruct UeF as [U1 [U2 [UeF [V1Sub U2Sub]]]].
    apply IHHs1 in UeF.
    destruct UeF as [U3 [U4 [SeF [U1Sub U4Sub]]]].
    exists U3, U4...
...
Proof with eauto.
  ...
  destruct H1 as [U1 [U2 [TeF [T1Sub U2Sub]]]].
  eauto.
  Qed.
(** [] *)
...
Proof.
  ...
  - destruct (String.eqb x x0) eqn:X; subst;
    ...
    rewrite X in H; inversion H; eauto.
    rewrite H1 in Hv.
