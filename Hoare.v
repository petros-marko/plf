...
Theorem hoare_post_true : forall (P Q : Assertion) c,
  ...
  {{P}} c {{Q}}.
Proof.
  unfold valid_hoare_triple. intros. apply H.
  Qed.
(** [] *)
...
Theorem hoare_pre_false : forall (P Q : Assertion) c,
  ...
  {{P}} c {{Q}}.
Proof.
  unfold valid_hoare_triple. intros. apply H in H1. destruct H1.
  Qed.
(** [] *)
...
Proof.
  ...
  specialize H with (st := (_ !-> 0)) (st' := (X !-> 1 ; _ !-> 0)).
  assert (A : (_ !-> 0) =[ X := X + 1 ]=> (X !-> 1; _ !-> 0)).
...
Proof.
  ...
  destruct H0.
  split;
  rewrite t_update_shadow, <- H1, t_update_same.
  - assumption.
...
Proof.
  ...
  apply H in A.
  - simpl in A. unfold t_update in A.
    rewrite String.eqb_refl in A. discriminate A.
  - reflexivity.
  Qed.
...
Inductive ceval : state -> com -> state -> Prop :=
  ...
  | E_WhileTrue : forall st st' st'' b c,
      ...
      st' =[ while b do c end ]=> st'' ->
      st  =[ while b do c end ]=> st''
  | E_RepeatUntilTrue : forall b st st' c,
      st =[ c ]=> st' ->
      beval st' b = true ->
      st =[ repeat c until b end ]=> st'
  | E_RepeatUntilFalse : forall b st st' st'' c,
      st  =[ c ]=> st' ->
      beval st' b = false ->
      st' =[ repeat c until b end ]=> st'' ->
      st  =[ repeat c until b end ]=> st''
...
Proof.
  unfold ex1_repeat, empty_st.
  eapply E_RepeatUntilTrue.
  - eapply E_Seq;
...
Proof.
  ...
  remember <{ repeat c until b end }> as original_cmd eqn:Horig.
  induction H0;
  try inversion Horig; subst.
  - apply H in H0; 
      [ split | idtac];
    assumption.
  - apply IHceval2 in Horig;
      [ idtac | apply H in H0_];
    assumption.
  Qed.
...
Proof.
  unfold valid_hoare_triple, havoc_pre, assertion_sub.
  intros. 
  inversion H; subst;
  eauto.
  Qed.
...
Theorem assert_assume_differ : exists (P:Assertion) b (Q:Assertion),
  ...
  /\ ~ ({{P}} assert b {{Q}}).
Proof.
  exists (X = 0)%assertion, <{ ~ X = 0 }>, (X = 0)%assertion.
  unfold valid_hoare_triple, not. simpl. split.
  {
    intros. inversion H; subst.
    simpl in H2. rewrite H0 in H2. inversion H2.
  }
  {
    intros.
    specialize H with (st := empty_st) (r := RError).
    assert (A : empty_st =[ assert (~ X = 0) ]=> RError).
      { apply E_AssertFalse. reflexivity. }
    apply H in A.
    - destruct A as [st' [ReN _]].
      congruence.
    - reflexivity.
  }
  Qed.
...
Proof.
  unfold valid_hoare_triple. intros.
  inversion H0; subst.
  apply H with (st := st) (r := RNormal st);
    [ constructor | idtac ];
  assumption.
  Qed.
...
Proof.
  ...
  - eapply hoare_seq.
    + eapply hoare_consequence_post.
      * apply (hoare_assert True).
      * unfold "->>". intros. apply I.
    + apply hoare_asgn.
  - eapply hoare_consequence_post.
    + apply hoare_assume.
    + unfold "->>", assertion_sub. simpl.
      intros. rewrite eqb_eq in H. destruct H. split.
      * apply I.
