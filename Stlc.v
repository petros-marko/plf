...
Inductive substi (s : tm) (x : string) : tm -> tm -> Prop :=
  ...
  | s_abs1 : forall T s',
      substi s x (<{ \ x:T, s' }>) (<{ \ x:T, s' }>)
  | s_abs2 : forall y T s' s'',
      x <> y ->
      substi s x s' s'' ->
      substi s x (<{ \ y:T, s' }>) (<{ \ y: T, s'' }>)
  | s_app : forall s1 s2 s1' s2',
      substi s x s1 s1' ->
      substi s x s2 s2' ->
      substi s x (<{ (s1)(s2) }>) (<{ (s1')(s2') }>)
  | s_true : substi s x (<{ true }>) (<{ true }>)
  | s_false : substi s x (<{ false }>) (<{ false }>)
  | s_if : forall c c' s1 s1' s2 s2',
      substi s x c c' ->
      substi s x s1 s1' ->
      substi s x s2 s2' ->
      substi s x (<{ if c then s1 else s2 }>) (<{ if c' then s1' else s2' }>)
.
...
Proof.
  ...
  generalize dependent t'.
  {
    induction t; intros;
    simpl in H.
    - destruct (String.eqb x0 s0) eqn:XeS.
      + apply String.eqb_eq in XeS. subst.
        constructor.
      + apply String.eqb_neq in XeS. subst.
        constructor. assumption.
    - rewrite <- H.
      apply s_app; auto.
    - destruct (String.eqb x0 s0) eqn:XeS.
      + apply String.eqb_eq in XeS.
        rewrite <- XeS, <- H in *.
        constructor.
      + apply String.eqb_neq in XeS.
        rewrite <- H.
        auto.
    - rewrite <- H. auto.
    - rewrite <- H. auto.
...
Proof.
  ...
  - rewrite t_update_permute.
    ...
    + unfold not. intros. solve_by_invert.
  - econstructor.
    + replace (y !-> Some <{ Bool -> Bool }>; x !-> Some <{ Bool -> Bool }>; empty)
      with (x !-> Some <{ Bool -> Bool }>; y !-> Some <{ Bool -> Bool }>; empty).
      * rewrite t_update_permute.
        { constructor. reflexivity. }
        { unfold not. intros. solve_by_invert. }
      * rewrite t_update_permute.
...
Example typing_nonexample_3 :
  ~ (exists S T,
        empty |--
          \x:S, x x \in T).
Proof.
  unfold not. intros. destruct H as [x0 [T]].
