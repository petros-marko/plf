...
Lemma bool_step_prop1_not_provable : ~ bool_step_prop1.
Proof.
...
Lemma bool_step_prop2_not_provable : ~bool_step_prop2.
Proof.
...
Lemma bool_step_prop3_provable : bool_step_prop3.
Proof.
...
Theorem short_circuit_non_deterministic : ~ deterministic step.
Proof.
  unfold deterministic, not. intros.
  assert (Contra : (test tru tru tru) = tru).
    {
      apply H with (x := (test (test tru tru tru) tru tru));
      repeat constructor.
    }
  solve_by_invert.
  Qed.
...
Theorem strong_progress_bool_short_circuit : forall t,
  value t \/ exists t', t --> t'.
Proof.
  induction t.
  - left. constructor.
...
Proof.
  ...
  induction H.
  - constructor.
  - assert (L: P t1 t2 -->* P (C n1) t2).
      { apply multistep_congr_1. assumption. }
    assert (R: P (C n1) t2 -->* P (C n1) (C n2)).
...
Proof.
  ...
  intros t t' [R N].
  induction R.
  - apply nf_is_value in N.
    inversion N; subst.
    exists n. split.
    { reflexivity. }
    { constructor. }
  - apply IHR in N.
    destruct N. destruct H0 as [Zeq Yev].
    exists x0. split.
    { assumption. }
    {
      eapply step__eval;
...
Proof.
  ...
  unfold par_loop.
  eapply multi_step.
  { apply CS_Par2, CS_While. }
  eapply multi_step.
  { apply CS_Par2, CS_IfStep, BS_Eq1, AS_Id. }
  eapply multi_step.
  { apply CS_Par2,CS_IfStep, BS_Eq. }
  rewrite H0, Nat.eqb_refl.
  eapply multi_step.
  { apply CS_Par2, CS_IfTrue. }
  eapply multi_step.
  { apply CS_Par2, CS_SeqStep, CS_AsgnStep, AS_Plus1, AS_Id. }
  eapply multi_step.
  { apply CS_Par2, CS_SeqStep, CS_AsgnStep, AS_Plus. }
  eapply multi_step.
  { apply CS_Par2, CS_SeqStep, CS_Asgn. }
  eapply multi_step.
  { apply CS_Par2, CS_SeqFinish. }
  rewrite Nat.add_1_r, H.
  apply multi_refl.
  Qed.
