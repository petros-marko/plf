...
Proof.
  induction t; unfold step_normal_form, not; intros;
  destruct H0; try solve_by_inverts 2.
  inversion H; inversion H1; subst.
  assert (S: step_normal_form t).
    { apply IHt. right. assumption. }
  unfold step_normal_form in *.
  inversion H0; subst. apply S.
...
Proof with eauto.
  ...
  - inversion H2; subst.
    assert (V : value v).
      { right. assumption. }
    apply (value_is_nf v) in V.
    assert (Contra : False).
      { apply V. exists t1'0. assumption. }
    contradiction.
  - inversion H; subst.
    assert (Y : value y2).
      { right. assumption. }
    apply (value_is_nf y2) in Y.
    assert (Contra : False).
      { apply Y. exists t1'0. assumption. }
    contradiction.
  - apply IHstep in H2. rewrite H2. reflexivity.
  - inversion H2; subst.
    assert (V : value v).
...
Proof.
  ...
  unfold not. intros.
  specialize H with
    (t  := <{ if false then 0 else true }>)
    (t' := <{ true }>)
    (T  := Bool) as H'.
  assert (Contra : |-- <{ if false then 0 else true }> \in Bool).
    { apply H'. split; constructor. }
  solve_by_inverts 2.
  Qed.
...
Theorem progress_bigstep : forall t T,
  |-- t \in T ->
  value t \/ exists t', t -->* t'.

Theorem preservation_bigstep : forall t t' T,
  |-- t \in T ->
  t -->* t' ->
  |-- t' \in T.
(* Do not modify the following line: *)
