...
Proof.
  ...
  - right.
    inversion Ht; subst.
    apply IHt1 in H2 as Ht1.
    apply IHt2 in H4.
    destruct Ht1.
    + destruct H4.
      * apply canonical_forms_fun in H2; [| assumption].
        destruct H2 as [x [u]]; subst;
        eauto.
      * destruct H0. eauto.
    + destruct H; eauto.
  - right.
    inversion Ht; subst.
    apply IHt1 in H3 as Ht1.
    apply IHt2 in H5.
    apply IHt3 in H6.
    destruct Ht1.
...
Proof.
  ...
  - destruct (String.eqb x x0) eqn:X; subst;
  ...
  - destruct (String.eqb x x0) eqn:X; subst.
    + constructor.
      rewrite String.eqb_eq in X.
      rewrite X, update_shadow in Ht.
      assumption.
    + constructor.
      apply IHHt with (Gamma := x0 |-> T2; Gamma').
      rewrite String.eqb_neq in X.
      rewrite update_permute; auto.
  Qed.
...
Proof.
  exists <{ (\y: Bool, true) (\z: Bool, true) }>, <{ true }>, <{ Bool }>.
  split; [| split]; auto.
  unfold not. intros.
  inversion H; subst.
...
Proof.
  ...
  - inversion H0. reflexivity.
  - inversion H2; subst.
    apply IHhas_type2 in H9. assumption.
  Qed.
...
Proof.
  ...
  - constructor.
    rewrite <- H0; auto.
  - constructor.
    apply IHhas_type with (Gamma' := x0 |-> T2; Gamma').
    intros. destruct (String.eqb x0 x1) eqn:X;
    unfold update, t_update; rewrite X;
    [ | rewrite String.eqb_neq in X];
    auto.
  - econstructor; auto.
  Qed.
...
and the following typing rule:
    ...
    false, give a counterexample.
      ...
      - Determinism of [step] becomes false.
        (\x0: Bool, false) true --> zap
        (\x0: Bool, false) true --> false
        But zap <> false.
...
(** **** Exercise: 2 stars, standard (stlc_variation2)
    ...
    false, give a counterexample.
      ...
      - Preservation becomes false
        |-- (\x: A, x) \in A -> A and
        (\x: A, x) --> foo but
        it is not the case that |-- foo \in A -> A
*)
...
(** **** Exercise: 2 stars, standard (stlc_variation3)
    ...
    false, give a counterexample.
      ...
      - Progress becomes false
        |-- ((\x: Bool, \y: Bool, x) true) ((\x: Bool, x) true) \in Bool -> Bool -> Bool and
        since it is ST_App it is not a value, but since we cannot reduce the left
        side of the application and the right side is not a value either,
        there is no step we can take
      - Preservation remains true
*)
...
(** **** Exercise: 2 stars, standard, optional (stlc_variation4)
    ...
    false, give a counterexample.
      ...
      - Progress remains true
      - Preservation becomes false
        |-- if true then (\x: Bool, x) else (\y: Bool, y) \in Bool -> Bool and
        if true then (\x: Bool, x) else (\y: Bool, y) --> true but
        it is not the case that |-- true \in Bool -> Bool
*)
...
(** **** Exercise: 2 stars, standard, optional (stlc_variation5)
    ...
    false, give a counterexample.
      - Determinism of [step] remains true
      - Progress becomes false
        |-- (\x: Bool, \y: Bool, x) \in Bool -> Bool -> Bool and
        |-- true \in \Bool so
        |-- (\x: Bool, \y: Bool, x) true \in Bool thus
        |-- if (\x: Bool, \y: Bool, x) true then true else true \in Bool
        if (\x: Bool, \y: Bool, x) true then true else true is not a value
        and cannot take a step
      - Preservation remains true
*)
...
(** **** Exercise: 2 stars, standard, optional (stlc_variation6)
    ...
    false, give a counterexample.
      ...
      - Progress becomes false
        |-- true \in Bool and 
        |-- true \in Bool so
        |-- true true \in Bool and hence
        |-- if true true then true else true \in Bool but
        if true true then true else true is not a value nor can it
        take a step
      - Preservation remains true
*)
...
(** **** Exercise: 2 stars, standard, optional (stlc_variation7)
    ...
    false, give a counterexample.
      ...
      - Progress becomes false
        |-- \x: Bool, x \in Bool so
        |-- if (\x: Bool, x) then true else true \in Bool
        but if (\x: Bool, x) then true else true is not a value nor can
        it take a step
      - Preservation becomes false
        |-- \x: Bool, \y: Bool -> Bool, x \in Bool
        (\x: Bool, \y: Bool -> Bool, x) true --> (\y: Bool -> Bool, true)
        but it is not the case that |-- (\y: Bool -> Bool, true) \in Bool
*)
...
Inductive has_type : context -> tm -> ty -> Prop :=
  ...
  | T_Succ : forall Gamma t,
      Gamma |-- t \in Nat -> Gamma |-- <{ succ t }> \in Nat
  | T_Pred : forall Gamma t,
      Gamma |-- t \in Nat -> Gamma |-- <{ pred t }> \in Nat
...
Proof.
  intros t T1 T2 HT HVal.
  destruct HVal as [x |] ; inversion HT; subst.
  exists x0, t. reflexivity.
Qed.
...
Proof with eauto.
  ...
  - right.
    ...
    + apply canonical_forms_nat in H; [| auto].
      destruct (IHHt2 eq_refl).
      * apply canonical_forms_nat in H0; [| auto].
        destruct H, H0. rewrite H,H0...
      * destruct H, H0. rewrite H...
    + destruct H...
  - (* T_If *)
    right. destruct IHHt1...

    + (* t1 is a value *)
      destruct (canonical_forms_nat t1); subst...
      destruct x0 eqn:X0...
